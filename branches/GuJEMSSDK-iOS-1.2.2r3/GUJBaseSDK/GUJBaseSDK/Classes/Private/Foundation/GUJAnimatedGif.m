...
@implementation GUJAnimatedGifQueueObject
...
@implementation GUJAnimatedGif

static GUJAnimatedGif * instance;

@synthesize imageView;
@synthesize busyDecoding;

+ (GUJAnimatedGif *) sharedInstance
{
    if (instance == nil)
    {
        instance = [[GUJAnimatedGif alloc] init];
    }
...
{   
    ...
    GUJAnimatedGifQueueObject *agqo = [[GUJAnimatedGifQueueObject alloc] init];
    ...
    [[GUJAnimatedGif sharedInstance] addToQueue: agqo];

    
    if ([[GUJAnimatedGif sharedInstance] busyDecoding] != YES)
    {
        [[GUJAnimatedGif sharedInstance] setBusyDecoding: YES];

        // Asynchronous loading for URL's, else the GUI won't appear until image is loaded.
        /*
         * changed to selectorOnMainThread cause we're in a framework
         * @sven
         */
        // we have to wait until the image is loaded, cause otherwise we cant get the correct size of the image.
        [[GUJAnimatedGif sharedInstance] performSelectorOnMainThread:@selector(asynchronousLoading) withObject:nil waitUntilDone:YES];
    }
...
- (void) asynchronousLoading
{
    ...
    {
    	NSData *data = [NSData dataWithContentsOfURL: [(GUJAnimatedGifQueueObject *) [imageQueue objectAtIndex: 0] url]];        
...
- (void) addToQueue: (GUJAnimatedGifQueueObject *) agqo
{
